...
const getUserSingleAnswer = AnswerService.getUserSingleAnswer
...
describe('Answer Single', () => {
    ...
    it('Calls axios with correct payload', async () => {
        ;(getUserSingleAnswer as jest.Mock).mockResolvedValueOnce(null)
        await factory()
        await flushPromises()
    ...
    it('Shows an error message when an answer is not found', async () => {
        ;(getUserSingleAnswer as jest.Mock).mockRejectedValueOnce(
            new Error('Not Found')
        )
        const wrapper = await factory()
        await flushPromises()
    ...
    it('Shows the correct answer title', async () => {
        ;(getUserSingleAnswer as jest.Mock).mockResolvedValueOnce({
            data: answers[0],
        })
        const wrapper = await factory()
        await flushPromises()
