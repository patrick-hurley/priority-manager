...
const getUserAnswers = AnswerService.getUserAnswers
...
describe('Answer List', () => {
    ...
    it('Shows an error when no user found', async () => {
        ;(getUserAnswers as jest.Mock).mockRejectedValueOnce(
            new Error('Not Found')
        )
    ...
    it('Shows the correct message when no results are found', async () => {
        ;(getUserAnswers as jest.Mock).mockResolvedValueOnce({ data: [] })
        const wrapper = await factory()
        await flushPromises()
    ...
    it('Shows a link list when results are returned', async () => {
        ;(getUserAnswers as jest.Mock).mockResolvedValueOnce({ data: answers })
        const wrapper = await factory()
        await flushPromises()
