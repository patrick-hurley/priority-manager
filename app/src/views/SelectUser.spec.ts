...
describe('SelectUser', () => {
    ...
    it('Calls getUsers and displays a dropdown when array is NOT empty', async () => {
        ;(getAllUsers as jest.Mock).mockResolvedValueOnce(resp)
    ...
    it('Displays an error message when getUsers call fails', async () => {
        ;(getAllUsers as jest.Mock).mockRejectedValueOnce(null)
        const wrapper = await factory()
        await flushPromises()
    ...
    it('Changes navigation when the button is clicked', async () => {
        ;(useRouter as jest.Mock).mockImplementationOnce(() => ({ push }))
        ;(getAllUsers as jest.Mock).mockResolvedValueOnce(resp)
